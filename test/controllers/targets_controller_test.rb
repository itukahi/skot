...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post targets_url, params: { target: { channelnumber: @target.channelnumber, janru: @target.janru } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { channelnumber: @target.channelnumber, janru: @target.janru } }
